...
from search import highlight_full_text, category_badge
...
sb_total_w = "170px" if is_exp else "84px"   # 8px left + sb_w + 8px gap
...
# ── SVG 아이콘 ────────────────────────────────────────────────────────────────
_svg_shield = ('<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>')
...
_nav_html = ""
for _np, _ni, _nl in [("search", _svg_search, "검색"), ("docs", _svg_docs, "문서관리")]:
    _fw = "700" if (page == _np) else "400"
    if is_exp:
        _nav_html += (
            f'<a href="/?page={_np}" target="_self" class="sb-link" style="{_lnk}'
...
# ── 사이드바: position:fixed 커스텀 div (st.sidebar 미사용) ──────────────────
st.markdown(f"""
<div style="
...
# ── 상단바 ────────────────────────────────────────────────────────────────────
_TOP_MENUS  = ["규정검색", "재무건전성비율", "위원회"]
_ACTIVE_TOP = "규정검색"

menu_pills = "".join(
    f'<span class="menu-pill-inactive" style="'
